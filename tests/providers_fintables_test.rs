    ...
    fn fetch_raw(
        ...
        for ds in datasets {
            if ds == "corporate_actions" || ds == "fundamentals" {
                let url = format!("{}/sirketler/{}/sermaye-artirimlari-temettuler", self.base, symbol);
                let resp = self.client.get(&url).send()?;
                let text = resp.text()?;
                out.insert(ds.clone(), json!([{"html": text, "source": "fintables", "symbol": symbol}]));
            }
        }
        Ok(out)
    }
...
fn fintables_fetch_via_mock() {
    ...
    let body = std::fs::read_to_string("tests/fixtures/fintables_example.html").expect("read fixture");
    let _m = server.mock(|when, then| {
        when.method(GET).path("/sirketler/BTCUSDT/sermaye-artirimlari-temettuler");
        then.status(200).header("Content-Type", "text/html").body(body);
    });

    let mut registry = SourceAdapterRegistry::default();
    registry.register("fintables", Arc::new(TestFintablesAdapter::new(server.base_url())));
